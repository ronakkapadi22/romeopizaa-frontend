...
const CartDetail = ({ className, ...props }) => {
	...
	console.log('totalTax', totalTax, cartItems)
	...
	const handlePaymentIntent = async () => {
		...
		setLoading(true)
		console.log('subTotal', subTotal, totalTax, discountData?.discountAmount)
		try {
			const response = await createPaymentIntent({
				amount: (Number(subTotal) + Number(totalTax) + 1 - discountData?.discountAmount) * 100,
				currency: 'gbp',
				metadata: {
					customerId: customer?.id || '',
